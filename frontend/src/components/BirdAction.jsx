...
const ImageGrid = ({ probabilities, selectedImages, onImageSelect, onSubmitSelections, onNoneSelection }) => {
    ...
    return (
        <div className="w-full p-6 max-w-3xl mx-auto bg-white rounded-xl shadow-md border border-slate-200 text-slate-800">
            <h3 className="text-2xl font-bold mb-5 text-center text-blue-700">
                Select 3 similar images <span className="text-blue-600">({selectedImages.length}/3)</span>
            </h3>

            <div className="mb-6 flex justify-center">
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', width: '100%', gap: '12px' }}>
                    {visibleImages.slice(0, 6).map((img, idx) => {
                        ...
                        const labelColor = isFirstClass ? 'bg-blue-600' : 'bg-emerald-600';
                        ...
                        return (
                            <div
                                ...
                                key={`img-${idx}`}
                                className={`relative cursor-pointer rounded-xl overflow-hidden border ${isSelected ? 'border-blue-600' : 'border-slate-200'}`}
                            ...
                            >
                                ...
                                />
                                <div className={`absolute bottom-0 left-0 right-0 ${labelColor} bg-opacity-90 py-2 text-center text-sm font-medium truncate z-20 text-white`}>
                                    {classLabel}
                                </div>
                                
                                {isSelected && (
                                    <>
                                        {/* Light pulsing overlay */}
                                        <div style={{
                                            position: 'absolute',
                                            inset: '0',
            ...
            <div className="flex flex-row items-center justify-center gap-4 sm:gap-5">
                <button
                    className={`px-6 sm:px-8 py-3 rounded-xl font-bold transition-all duration-300 ${
                ...
                <button
                    className="px-6 sm:px-8 py-3 bg-red-500 hover:bg-red-600 text-white rounded-xl font-bold transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-red-500/20"
...
const BirdAction = () => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-blue-50 to-slate-100 pt-16 pb-20" id="process">
            <div className="container mx-auto px-4">
                <div className="rounded-xl p-8 md:flex items-center w-full md:flex-row justify-around bg-white/80 shadow-lg backdrop-blur-3xl border border-slate-200">
                    <Dropzone onDropZoneInputChange={onDropZoneInputChange} />
                    
                    <div className="max-md:w-full max-md:mt-8 md:ml-8 w-full md:w-3/5 rounded-xl flex justify-center items-center h-fit border border-slate-200 bg-white shadow-md p-6 text-slate-800 font-medium transition-all duration-300 hover:border-blue-200 overflow-auto">
                        ...
                        {error && (
                            <div className="bg-red-50 border border-red-300 rounded-lg p-4 w-full text-center">
                        ...
                        {result && !probabilities && (
                            <div className="text-slate-800 flex flex-col justify-center w-full">
                                {result.isBird !== undefined ? (
                                    <div className={`text-center p-6 rounded-xl ${result.isBird ? 'bg-green-50 border border-green-300' : 'bg-amber-50 border border-amber-300'}`}>
                                        <svg className={`w-16 h-16 mx-auto mb-4 ${result.isBird ? 'text-green-500' : 'text-amber-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            {result.isBird ? 
                                    ...
                                    <div className="self-center flex flex-col items-center w-full">
                                        {result.s3ImageUrl && (
                                            <div className="my-6 rounded-xl overflow-hidden shadow-md border border-slate-200 flex justify-center">
                                                <img
                                        ...
                                        {result.initial_prediction && result.final_prediction ? (<>
                                            <div className="mt-6 text-center flex flex-col md:flex-row justify-center items-stretch gap-6 w-full">
                                                <div className="bg-slate-50 rounded-xl p-5 flex-1 border-l-4 border-blue-500 shadow-md hover:shadow-blue-500/10 transition-all duration-300">
                                                    <p className="font-bold text-xl mb-2 text-slate-800">Pre-User Prediction: <span className="text-blue-600">{result.initial_prediction.class}</span></p>
                                                </div>
                                                
                                                <div className="bg-slate-50 rounded-xl p-5 flex-1 border-l-4 border-green-500 shadow-md hover:shadow-green-500/10 transition-all duration-300">
                                                    <p className="font-bold text-xl mb-2 text-slate-800">Final Prediction: <span className="text-green-600">{result.final_prediction.class}</span></p>
                                                </div>
                                            </div>
                                            {result.final_prediction && <BirdInfoDisplay name={result.final_prediction.class}/>}
                                            </>
                                        ) : (
                                            <div className="mt-6 text-center p-6 bg-blue-50 border border-blue-300 rounded-xl w-full">
                                                <p className="text-xl font-bold mb-2 text-slate-800">Classification Result:</p>
                ...
                <div className="flex justify-center items-center p-12">
                    {loading ? (
                        <div className="flex flex-col items-center">
                            ...
                            />
                            <p className="mt-4 text-slate-800 text-xl">Processing your image...</p>
                        </div>
                    ) : (
                        <div className="flex flex-col items-center space-y-8">
                            <div className="flex flex-wrap justify-center gap-4">
                                <button
                                    ...
                                    className="px-6 py-3 bg-blue-600 text-white rounded-xl font-medium shadow-md hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-500/20 transition-all duration-300 hover:scale-105 flex items-center"
                                >
                                ...
                                <button
                                    ...
                                    className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-medium shadow-md hover:bg-indigo-700 hover:shadow-lg hover:shadow-indigo-500/20 transition-all duration-300 hover:scale-105 flex items-center"
                                >
                                    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
