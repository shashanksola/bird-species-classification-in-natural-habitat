...
async function getAdjustedPredictions(
  birdUrl,
...
const ImageGrid = ({
  ...
  const handleImageSelect = (img) => {
    ...
    const element = document.getElementById(
      `img-container-${visibleImages.indexOf(img)}`
    );
    if (element) {
  ...
  return (
    <div className="w-full p-6 max-w-3xl mx-auto bg-indigo-900 rounded-xl border border-indigo-700 backdrop-blur-lg text-white">
      ...
      <div className="mb-6 flex justify-center">
        ...
        >
          {visibleImages.slice(0, 6).map((img, idx) => {
            ...
            const classLabel = isFirstClass
              ? probabilities.className
              : probabilities.topPrediction2_class;
            const labelColor = isFirstClass
            ...
            return (
              ...
              >
                ...
                />
                <div
                  className={`absolute bottom-0 left-0 right-0 ${labelColor} bg-opacity-85 py-2 text-center text-sm font-medium truncate z-20`}
                >
                  {classLabel}
                </div>
      ...
      <div className="w-full bg-indigo-700 h-3 rounded-full mb-6 overflow-hidden">
        <div
          className="bg-gradient-to-r from-purple-400 to-indigo-500 h-full transition-all duration-500 ease-out"
...
const BirdAction = () => {
  ...
  // Custom button styles with light theme
  ...
  return (
    <div
      className="min-h-screen bg-gradient-to-b from-blue-50 to-slate-100 bg-fixed bg-center pt-16 pb-20"
      id="process"
    >
                  ...
                  >
                    <svg
                      className={`w-16 h-16 mx-auto mb-4 ${
                        result.isBird ? "text-green-500" : "text-amber-500"
                      }`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    ...
                    <p className="text-xl font-bold">
                      {t("birdAction.ValidationResult")}{" "}
                      {result.isBird
                        ? "Contains Bird"
                        : "Doesn't contain any Bird"}
                    </p>
                  </div>
                ) : (
                    ...
                    {result.initial_prediction && result.final_prediction ? (
                      <>
                        <div className="mt-6 text-center flex flex-col md:flex-row justify-center items-stretch gap-6 w-full">
                          <div className="bg-slate-50 rounded-xl p-5 flex-1 border-l-4 border-yellow-400 shadow-md hover:shadow-yellow-400/10 transition-all duration-300">
                            <p className="font-bold text-xl mb-2 text-slate-800">
                              Pre-User Prediction:{" "}
                              <span className="text-slate-700">
                                {result.initial_prediction.class}
                              </span>
                            </p>
                          </div>

                          <div className="bg-slate-50 rounded-xl p-5 flex-1 border-l-4 border-green-400 shadow-md hover:shadow-green-400/10 transition-all duration-300">
                            <p className="font-bold text-xl mb-2 text-slate-800">
                        ...
                        {result.final_prediction && (
                          <BirdInfoDisplay
                            name={result.final_prediction.class}
                          />
                        )}
                      </>
                    ) : (
        ...
        <div className="flex justify-center items-center p-12">
          {loading ? (
            <div className="flex flex-col items-center">
              ...
              />
              <p className="mt-4 text-slate-800 text-xl">
